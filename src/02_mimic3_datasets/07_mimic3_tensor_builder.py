...
DATA_DIR = BASE_DIR / "data/processed/mimic3-carevue"
...
def build_ventilation(connection, sofa_policy):
    ...
    connection.execute(f"""
        ...
        WITH labels AS (
            ...
            FROM cleaned WHERE feature IN ('oxygen_device', 'ventilator_mode', 'airway_status', 'extubation_status')
            ...
            SELECT stay_id, feature, label, ambiguous, event_time AS interval_start,
                LEAST(next_time, event_time + INTERVAL {freshness} HOUR,
                    CASE WHEN feature='airway_status' THEN (
                        SELECT MIN(barrier.event_time) FROM labels barrier
                        WHERE barrier.stay_id=durations.stay_id AND barrier.feature='extubation_status'
                            AND (barrier.label IN ('extubated','self extubation') OR barrier.ambiguous)
                            AND barrier.event_time>=durations.event_time
                    ) END) AS interval_end FROM durations
            WHERE feature <> 'extubation_status'
        )
    ...
    connection.execute(f"""
        ...
        WITH evidence AS (
            SELECT segments.*,
                ...
                COALESCE(BOOL_OR(support.feature = 'airway_status'
                    AND support.label = 'intubated/trach'), FALSE)
                    AND COALESCE(BOOL_OR(support.feature = 'oxygen_device'
                    AND support.label = 'ventilator'), FALSE) AS airway,
...
def build_tensor(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR, common_dir=COMMON_DIR,
    ...
    qc = importlib.import_module("09_mimic3_master_qc")
    ...
    with tempfile.TemporaryDirectory(prefix=".tensor_", dir=output_dir) as temporary:
        ...
        paths += [Path(__file__).with_name(name) for name in
                  ("01_mimic3_base_cohort.py", "02_mimic3_infection_cohort.py", "03_mimic3_phenotypes.py",
                   "04_mimic3_temporal_extract.py", "05_mimic3_temporal_clean.py", "06_mimic3_sepsis3_cohort.py", "07_mimic3_tensor_builder.py",
                   "09_mimic3_master_qc.py")]
