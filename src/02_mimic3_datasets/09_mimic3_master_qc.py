"""Audit the MIMIC processing outputs and replay the selected imputation recipe."""
import argparse
import contextlib
import csv
import hashlib
...
DATA = ROOT / "data/processed/mimic3-carevue"
PROCESSED_DIR = DATA / "work"
OUT = METRICS_DIR = ROOT / "outputs/mimic3-carevue"
...
def artifact_checks(qc, state):
    ...
    script = Path(__file__).with_name("08_mimic3_saits_imputation.py")
    ...
    core = module(script, "mimic_imputation")
...
def observed_replay(qc, state):
    builder = module(Path(__file__).with_name("07_mimic3_tensor_builder.py"), "mimic_tensor_builder")
...
def audit_cohort(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR, common_dir=COMMON_DIR,
    ...
    with tempfile.TemporaryDirectory(prefix=".cohort_audit_", dir=metrics_dir) as temporary:
        ...
        try:
            ...
            with duckdb.connect() as connection:
                ...
                add_check(checks,'diagnostic_specimen_mapping',connection.execute(f"""
                    SELECT COUNT(*) FROM candidates c LEFT JOIN allowed_cultures a USING(spec_itemid)
                    WHERE a.spec_itemid IS NULL OR {normalized_label_sql('c.spec_type_desc')} = ''
                        ...
                        OR c.diagnostic_evidence_basis IS DISTINCT FROM 'specimen_proxy_no_test_field'
                """).fetchone()[0])
                carevue_checks(connection, checks)
        ...
        report = {
            ...
            "source_specific_limits": CONFIG['carevue'],
...
def carevue_checks(connection, checks):
    def check(name, query):
        add_check(checks,name,connection.execute(query).fetchone()[0])

    check('required_vital_channels_survive_cleaning', """
        SELECT COUNT(*) FROM (VALUES ('rr'), ('temp_c')) AS needed(feature)
        WHERE NOT EXISTS (SELECT 1 FROM cleaned c WHERE c.feature=needed.feature AND c.numeric_usable)
    """)
    check('carevue_rr_temperature_unit_conversions', """
        SELECT COUNT(*) FROM cleaned WHERE source_table='chartevents' AND (
            (unit_rule IN ('carevue_respiratory_bpm','carevue_temperature_c')
                AND converted_value IS DISTINCT FROM raw_valuenum)
            OR (unit_rule='carevue_temperature_f'
                AND ABS(converted_value-(raw_valuenum-32.0)/1.8)>1e-9))
    """)

    check('carevue_source',"""
        SELECT COUNT(*) FROM cleaned WHERE source_db IS DISTINCT FROM 'MIMIC-III CareVue'
            OR source_table NOT IN ('chartevents','labevents','inputevents','outputevents')
            OR ((numeric_usable OR evidence_usable) AND (source_error OR infusion_rate_conflict))
    """)
    check('carevue_first_stay',"""
        SELECT COUNT(*) FROM final WHERE LOWER(dbsource) IS DISTINCT FROM 'carevue'
            OR icu_seq IS DISTINCT FROM 1 OR age < 18
            OR (age_is_deidentified AND age <> 91)
    """)
    check('prescription_date_intervals',"""
        SELECT COUNT(*) FROM candidates WHERE abx_start_time IS NULL
            OR abx_start_time IS DISTINCT FROM DATE_TRUNC('day',abx_start_time)
            OR abx_start_time_upper_bound IS DISTINCT FROM abx_start_time + INTERVAL 1 DAY
            OR abx_start_time_upper_exclusive IS DISTINCT FROM TRUE
            OR abx_start_time_is_date_only IS DISTINCT FROM TRUE
            OR abx_duration_hours IS NOT NULL OR abx_duration_complete IS DISTINCT FROM FALSE
    """)
    check('interval_pairing_definiteness',"""
        SELECT COUNT(*) FROM candidates WHERE pairing_is_definite IS DISTINCT FROM
            (culture_time >= abx_start_time_upper_bound - INTERVAL 72 HOUR
                AND culture_time_upper_bound <= abx_start_time + INTERVAL 24 HOUR)
            OR culture_time >= abx_start_time_upper_bound + INTERVAL 24 HOUR
            OR culture_time_upper_bound < abx_start_time - INTERVAL 72 HOUR
            OR (culture_time_upper_bound = abx_start_time - INTERVAL 72 HOUR AND culture_time_upper_exclusive)
    """)
    check('interval_pairing_projections',"""
        SELECT COUNT(*) FROM candidates WHERE
            c_lo IS DISTINCT FROM GREATEST(culture_time,abx_start_time - INTERVAL 72 HOUR)
            OR c_hi IS DISTINCT FROM LEAST(culture_time_upper_bound,abx_start_time_upper_bound + INTERVAL 24 HOUR)
            OR a_lo IS DISTINCT FROM GREATEST(abx_start_time,culture_time - INTERVAL 24 HOUR)
            OR a_hi IS DISTINCT FROM LEAST(abx_start_time_upper_bound,culture_time_upper_bound + INTERVAL 72 HOUR)
            OR suspected_infection_time IS DISTINCT FROM LEAST(a_lo,c_lo)
            OR suspected_infection_time_upper_bound IS DISTINCT FROM LEAST(a_hi,c_hi)
            OR suspected_infection_time_upper_exclusive IS DISTINCT FROM
                ((a_open AND a_hi<=c_hi) OR (c_open AND c_hi<=a_hi))
    """)
    check('lab_grouping_disclosed',"""
    ...
    """)
    check('arterial_confirmation',"""
        SELECT COUNT(*) FROM cleaned WHERE numeric_usable AND feature IN ('pao2','paco2')
            AND (specimen_type_conflict OR specimen_type IS NULL
                OR UPPER(TRIM(specimen_type)) NOT IN ('ART.','ART','ARTERIAL'))
    """)
    check('infusion_endpoint_and_weight_evidence',"""
        SELECT COUNT(*) FROM cleaned WHERE source_table='inputevents' AND numeric_usable
            AND (event_end_time IS NULL OR event_end_time<=event_time
                OR event_end_time>event_time+INTERVAL 4 HOUR
                OR LOWER(TRIM(COALESCE(source_status,'')))='stopped'
                OR (unit_rule='documented_weight_conversion' AND (
                    weight_observation_time IS NULL OR weight_observation_time>event_time
                    OR weight_observation_time<event_time-INTERVAL 24 HOUR)))
    """)
    ...
    with duckdb.connect() as con:
        ...
        queries = dict(
            first_icu_rank="""SELECT COUNT(*) FROM (SELECT *, ROW_NUMBER() OVER
                (PARTITION BY subject_id ORDER BY icu_intime, stay_id) AS expected FROM eligibility)
            ...
            age_formula="""SELECT COUNT(*) FROM base WHERE age IS DISTINCT FROM
                CASE WHEN DATE_PART('year', AGE(hospital_admittime, dob)) >= 300 THEN 91
                ELSE DATE_PART('year', AGE(hospital_admittime, dob)) END""",
