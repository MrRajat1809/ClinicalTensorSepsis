...
OUT_DIR = BASE_DIR / "data" / "processed" / "mimic3-carevue" / "work"
...
def build_base_cohort(mimic_dir=MIMIC_DIR, out_dir=OUT_DIR, metrics_dir=METRICS_DIR):
    ...
    sources = {
        "source_icu": mimic_dir / "ICUSTAYS.csv.gz",
        "source_patients": mimic_dir / "PATIENTS.csv.gz",
        "source_admissions": mimic_dir / "ADMISSIONS.csv.gz",
    }
    for path in sources.values():
        if not path.is_file():
            raise FileNotFoundError(f"Required MIMIC-III CareVue source not found: {path}")


    with duckdb.connect(database=":memory:") as connection:
        connection.execute(f"""
            CREATE TEMP TABLE source_icu AS
            SELECT subject_id, hadm_id, icustay_id AS stay_id, dbsource, intime, outtime, los, first_careunit
            FROM read_csv_auto({sql_path(sources['source_icu'])}, types={{
                'subject_id': 'BIGINT', 'hadm_id': 'BIGINT', 'icustay_id': 'BIGINT',
        ...
        connection.execute(f"""
            ...
            FROM read_csv_auto({sql_path(sources['source_patients'])}, types={{
                'subject_id': 'BIGINT', 'dob': 'TIMESTAMP', 'dod': 'TIMESTAMP'
            }})
        """)
        connection.execute(f"""
            CREATE TEMP TABLE source_admissions AS
            SELECT subject_id, hadm_id, admittime, dischtime, deathtime,
                   ethnicity AS race, admission_type, hospital_expire_flag
        ...
        """)
        if connection.execute("SELECT COUNT(*) FROM source_icu WHERE LOWER(dbsource) IS DISTINCT FROM 'carevue'").fetchone()[0]:
            raise ValueError('Expected the CareVue-only source release')
        ...
        connection.execute("""
            ...
            WITH ranked_icu AS (
                ...
                SELECT
                    ...
                    patients.gender,
                    CASE WHEN DATE_PART('year', AGE(admissions.admittime, patients.dob)) >= 300 THEN 91
                        ELSE DATE_PART('year', AGE(admissions.admittime, patients.dob)) END AS age,
                    ...
                    patients.dob, ranked.dbsource,
                    DATE_PART('year', AGE(admissions.admittime, patients.dob)) >= 300 AS age_is_deidentified,
    ...
    report = {
        ...
        "source_database": "MIMIC-III CareVue", "source_version": "1.4",
        "cohort_specification_version": "2.0.0",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "duckdb_version": duckdb.__version__,
        "policy": {
            "first_stay": "first ICU stay per subject within supplied CareVue subset before eligibility filtering",
            ...
            "age": "Completed years from DOB at admission; shifted ages >=300 represented as 91 and flagged",
