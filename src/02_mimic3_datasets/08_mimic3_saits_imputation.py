...
PROCESSED_DIR = BASE_DIR / "data/processed/mimic3-carevue"
...
def load_inputs(processed_dir=PROCESSED_DIR, common_dir=COMMON_DIR):
    ...
    if (manifest.get('source_database') != CONFIG['source_database']
            or manifest.get('source_version') != CONFIG['source_version']):
        raise ValueError('Observed manifest is not the configured CareVue source release')
