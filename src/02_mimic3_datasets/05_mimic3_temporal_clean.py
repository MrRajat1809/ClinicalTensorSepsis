...
def configure_rules(connection, registry, rules):
    ...
    for feature in registry['sources']['inputevents']:
        for source_unit, target_unit in CONFIG['carevue']['rate_unit_aliases'].items():
...
def validate_inputs(connection):
    ...
    invalid_rows = connection.execute("""
        ...
        WHERE mapping.itemid IS NULL OR extraction_window.stay_id IS NULL
            OR raw.source_db IS DISTINCT FROM 'MIMIC-III CareVue'
    ...
    return f"""
        WITH prepared AS (
            ...
            SELECT prepared.*, conversions.factor, conversions.unit_offset, conversions.weight_operation,
                CASE
                    ...
                    WHEN source_table = 'chartevents' AND feature = 'rr' AND itemid = 618
                        AND normalized_unit = 'bpm' THEN 'carevue_respiratory_bpm'
                    WHEN source_table = 'chartevents' AND feature = 'temp_c' AND itemid = 676
                        AND normalized_unit = 'deg. c' THEN 'carevue_temperature_c'
                    WHEN source_table = 'chartevents' AND feature = 'temp_c' AND itemid = 678
                        AND normalized_unit = 'deg. f' THEN 'carevue_temperature_f'
                    WHEN feature = 'fio2' AND itemid IN (190, 3420, 3422, 50816)
            ...
            SELECT * EXCLUDE (factor, unit_offset, weight_operation),
                CASE
                    WHEN unit_rule IN ('mimic_fio2_fraction_recording', 'intrinsic_item_unit',
                        'mimic_mchc_percent_label', 'mimic_rbc_million_per_ul',
                        'carevue_respiratory_bpm', 'carevue_temperature_c') THEN raw_valuenum
                    WHEN unit_rule = 'carevue_temperature_f' THEN (raw_valuenum - 32.0) / 1.8
        ...
        SELECT * EXCLUDE (is_evidence, is_interval, in_icu, in_hospital, in_envelope, after_death),
            ...
            unit_rule IN ('mimic_fio2_fraction_recording', 'mimic_fio2_percent_recording',
                ...
                'carevue_respiratory_bpm', 'carevue_temperature_c', 'carevue_temperature_f')
...
def clean_temporal_data(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR,
    ...
    with tempfile.TemporaryDirectory(prefix=".clean_", dir=processed_dir) as temporary:
        ...
        with duckdb.connect() as connection:
            ...
            lost_vitals = {'rr', 'temp_c'} - accepted_features
            if lost_vitals:
                raise ValueError(f"Required CareVue vitals lost during cleaning: {sorted(lost_vitals)}; review source-unit rules")
            missing_features = [
        ...
        report = {
            ...
            "policy": {
                ...
                "carevue_vital_units": "Item 618 BPM is respiratory breaths/min; item 676 Deg. C is Celsius; item 678 Deg. F converts as (value-32)/1.8. Exceptions are item- and source-specific.",
