"""Adjudicate the eICU infection-proxy/SOFA cohort and strict culture sensitivity."""
...
def validate_inputs(connection):
    ...
    """, "Invalid documented-infection proxy/timing contract; rerun infection selection")
...
def build_respiratory(connection, policy):
    ...
    connection.execute(f"""
        ...
        WITH paired AS (
            ...
            SELECT *,
                COALESCE(device_recent AND device IN ('endotracheal tube', 'tracheostomy tube')
...
def build_timeline(connection, policy):
    ...
    connection.execute("""
        ...
        WITH times AS (
            ...
            UNION SELECT stay_id, suspected_infection_time FROM strict_candidates
            UNION SELECT stay_id, suspected_infection_time - INTERVAL 48 HOUR FROM strict_candidates
            UNION SELECT stay_id, suspected_infection_time + INTERVAL 24 HOUR FROM strict_candidates
            UNION
...
def build_strict_sensitivity(connection):
    """Re-adjudicate culture-paired candidates at their own SIT; do not reuse proxy onset."""
    connection.execute("""CREATE TEMP TABLE strict_decisions AS
        WITH first_scores AS (
            SELECT c.stay_id,c.infection_candidate_rank,MIN(t.assessment_time) AS first_qualifying_sofa_time
            FROM strict_candidates c LEFT JOIN timeline t ON c.stay_id=t.stay_id
                AND t.in_icu_assessment AND t.observed_components>0 AND t.total_sofa>=2
                AND t.assessment_time BETWEEN c.suspected_infection_time-INTERVAL 48 HOUR
                    AND c.suspected_infection_time+INTERVAL 24 HOUR
                AND GREATEST(c.suspected_infection_time,t.assessment_time)<=LEAST(c.icu_outtime,c.hospital_deathtime)
            GROUP BY c.stay_id,c.infection_candidate_rank
        ) SELECT c.*,s.first_qualifying_sofa_time,
            CASE WHEN s.first_qualifying_sofa_time IS NOT NULL
                THEN GREATEST(c.suspected_infection_time,s.first_qualifying_sofa_time) END AS strict_sepsis_onset_time,
            t.total_sofa AS strict_sofa, s.first_qualifying_sofa_time IS NOT NULL AS strict_qualifies
        FROM strict_candidates c JOIN first_scores s USING(stay_id,infection_candidate_rank)
        LEFT JOIN timeline t ON c.stay_id=t.stay_id AND t.assessment_time=s.first_qualifying_sofa_time""")
    connection.execute("""CREATE TEMP TABLE strict_selected AS SELECT * FROM strict_decisions WHERE strict_qualifies
        QUALIFY ROW_NUMBER() OVER(PARTITION BY stay_id ORDER BY infection_candidate_rank,first_qualifying_sofa_time)=1""")
    connection.execute("""CREATE TEMP TABLE final_with_sensitivity AS SELECT f.*,
        s.stay_id IS NOT NULL AS strict_culture_sepsis3,
        s.suspected_infection_time AS strict_culture_sit,
        s.strict_sepsis_onset_time AS strict_culture_onset_time,
        s.strict_sofa AS strict_culture_sofa,
        s.infection_candidate_rank AS strict_culture_candidate_rank,
        CASE WHEN s.stay_id IS NOT NULL THEN s.strict_sepsis_onset_time=f.sepsis_onset_time END AS strict_onset_matches_primary
        FROM final_cohort f LEFT JOIN strict_selected s USING(stay_id)""")
    connection.execute('DROP TABLE final_cohort')
    connection.execute('ALTER TABLE final_with_sensitivity RENAME TO final_cohort')
...
def build_sepsis_cohort(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR,
    ...
    inputs = {
        ...
        "strict_source_candidates": processed_dir / "strict_infection_candidates.parquet",
    }
    ...
    with tempfile.TemporaryDirectory(prefix=".05_", dir=processed_dir) as temporary:
        ...
        with duckdb.connect() as connection:
            ...
            connection.execute('CREATE TEMP TABLE strict_candidates AS SELECT s.* FROM strict_source_candidates s JOIN cohort c USING(stay_id)')
            ...
            build_strict_sensitivity(connection)
            require_zero(connection, """
            ...
            summary_queries = {
                ...
                "strict_culture_sepsis3_subgroup": "SELECT COUNT(*) FROM final_cohort WHERE strict_culture_sepsis3",
            ...
            outputs = {
                "strict_culture_decisions.parquet": "strict_decisions",
        ...
        report = {
            ...
            "limitations": [
                "Main eICU membership uses documented infection plus IV orders and operational SOFA, not the MIMIC culture-required definition. Infection-time proxy is the recorded IV order start; documentation does not establish clinical onset or exclude later ruled-out diagnoses.",
                "strict_culture_sepsis3 is re-adjudicated using strict culture SIT and its own SOFA window. The released tensor remains anchored to primary proxy onset; use strict onset metadata when constructing a strict-onset analysis.",
    ...
    print(f"[06] Complete v{DATASET_VERSION}; main proxy+SOFA={summary['sepsis3_stays']:,}; "
          f"strict culture+SOFA subgroup={summary['strict_culture_sepsis3_subgroup']:,}; "
          f"{time.perf_counter() - started:.1f}s; report: {metrics_dir}/06_sepsis.json", flush=True)
    return report


if __name__ == "__main__":
    print(f"[06] Adjudicate infection-proxy + SOFA cohort and strict culture sensitivity. v{DATASET_VERSION}", flush=True)
    build_sepsis_cohort()
