...
# Numerically equivalent lab-unit spellings; shared by extraction's conflict test.
LAB_UNIT_ALIASES = {
    "10^9/L": ["K/mcL", "K/mm3", "K/cumm", "K/cmm", "k/mm cu", "th/mm3", "th/uL", "th/cmm",
               "thou/cmm", "1000/cmm", "10^3/uL", "10*3/uL", "10e3/uL", "10e3/mcL", "x10e3/uL",
               "x10^3/uL", "*10^3/uL", "x10 3/uL", "x(10)3/uL", "10(3)/mcL", "thou/uL"],
    "10^12/L": ["M/mcL", "M/uL", "M/mm3", "M/cumm", "M/cmm", "m/mm cu", "mil/mm3", "mil/mcL",
                "mil/uL", "mill/uL", "mill/cmm", "10^6/uL", "10*6/uL", "10e6/uL", "10e6/mcL",
                "x10e6/uL", "x10^6/uL", "*10^6/uL", "x(10)6/uL", "x10 6/uL", "10(6)/mcL"],
    "s": ["second(s)", "secs"],
    "mmHg": ["mm/Hg"],
    "g/dL": ["gm/dL"],
    "mmol/L": ["mmoll"],
...
def lab_unit_signatures(registry, rules):
    """Compare two source units by feature-specific numeric conversion, never by spelling alone."""
    units = {entry['name']: entry['canonical_unit'] for entry in registry['temporal_features']}
    signatures = {}
    key_unit = lambda value: re.sub(r'\s+', '', normalized_unit(value))
    for feature in CONFIG['source_mapping']['sources']['labevents']:
        canonical = units[feature]
        for unit in rules['unit_aliases'][canonical] + LAB_UNIT_ALIASES.get(canonical, []):
            signatures[feature, key_unit(unit)] = (1.0, 0.0, 'none')
        for rule in rules['extra_conversions']:
            if feature in rule['features']:
                for unit in rule['units']:
                    key = (feature, key_unit(unit))
                    value = (float(rule['factor']), float(rule.get('offset', 0)), rule.get('weight_operation', 'none'))
                    if key in signatures and signatures[key] != value:
                        raise ValueError(f'Conflicting lab conversion: {key}')
                    signatures[key] = value
    return [(feature, unit, json.dumps(value)) for (feature, unit), value in sorted(signatures.items())]
...
def configure_rules(connection, registry, rules):
    ...
    mapping = []
    seen = set()
    for source, features in registry["sources"].items():
    ...
    for feature in {row[2] for row in mapping if row[3] is not None}:
        for unit in rules["unit_aliases"][units[feature]]:
            add(feature, unit)
        for unit in LAB_UNIT_ALIASES.get(units[feature], []):
            if (feature, normalized_unit(unit)) not in conversions:
                add(feature, unit)
    ...
    for feature in registry['sources']['inputevents']:
        for source_unit, target_unit in CONFIG['eicu']['rate_unit_aliases'].items():
            target = conversions.get((feature, target_unit))
            if target is not None:
                existing = conversions.get((feature, normalized_unit(source_unit)))
                if existing is None:
                    add(feature, source_unit, *target[2:])
                elif existing[2:] != target[2:]:
                    raise ValueError(f'Conflicting source rate alias: {feature}/{source_unit}')
    connection.execute("""
...
def validate_inputs(connection):
    ...
    invalid_rows = connection.execute("""
        ...
        WHERE mapping.itemid IS NULL OR extraction_window.stay_id IS NULL
            OR raw.source_db IS DISTINCT FROM 'eICU-CRD'
    ...
    return f"""
        WITH prepared AS (
            SELECT raw.*,
                ...
                LOWER(REPLACE(REPLACE(TRIM(COALESCE(NULLIF(raw.raw_unit, ''), raw.item_unit, '')), 'µ', 'u'), 'μ', 'u'))
                    AS normalized_unit,
            ...
            SELECT prepared.*, conversions.factor, conversions.unit_offset, conversions.weight_operation,
                CASE
                    ...
                    WHEN feature = 'fio2' AND source_table IN ('chartevents', 'labevents')
                        AND normalized_unit IN ('', '%', 'percent')
                        AND raw_valuenum BETWEEN 0.2 AND 1 THEN 'eicu_fio2_fraction_recording'
                    WHEN feature = 'fio2' AND source_table IN ('chartevents', 'labevents')
                        AND normalized_unit IN ('', '%', 'percent')
                        AND raw_valuenum BETWEEN 20 AND 100 THEN 'eicu_fio2_percent_recording'
            ...
            SELECT * EXCLUDE (factor, unit_offset, weight_operation),
                CASE
                    WHEN unit_rule IN ('eicu_fio2_fraction_recording', 'intrinsic_item_unit') THEN raw_valuenum
                    WHEN unit_rule = 'eicu_fio2_percent_recording' THEN raw_valuenum / 100.0
                ...
                CASE
                    WHEN COALESCE(source_unit_conflict, FALSE) THEN 'conflicting_source_units'
                    WHEN COALESCE(source_error, FALSE) THEN 'source_error'
                    ...
                    WHEN source_table = 'inputevents' AND LOWER(TRIM(COALESCE(source_status,''))) = 'stopped'
                        THEN 'infusion_stop_marker'
                    WHEN COALESCE(infusion_rate_conflict, FALSE) THEN 'conflicting_infusion_rates'
            ...
            SELECT *,
                CASE
                    WHEN COALESCE(source_value_conflict, FALSE) THEN 'conflicting_or_ambiguous_source_values'
                    ...
                    WHEN unit_rule IN ('unsupported_unit', 'ambiguous_fio2',
                        'missing_or_invalid_weight') THEN unit_rule
        ...
        SELECT * EXCLUDE (is_evidence, is_interval, in_icu, in_hospital, in_envelope, after_death),
            ...
            unit_rule IN ('eicu_fio2_fraction_recording', 'eicu_fio2_percent_recording',
                'intrinsic_item_unit')
...
def clean_temporal_data(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR,
    ...
    if read_json(extraction_report).get("lab_unit_aliases") != LAB_UNIT_ALIASES:
        raise ValueError("Lab-unit aliases changed since extraction; rerun stage 04")
    ...
    with tempfile.TemporaryDirectory(prefix=".clean_", dir=processed_dir) as temporary:
        ...
        with duckdb.connect() as connection:
            ...
            """).fetchall())
            connection.execute(f"""
                COPY (SELECT feature,itemid,raw_value,qc_status,COUNT(*) AS rows,
                    COUNT(DISTINCT stay_id) AS stays FROM cleaned WHERE canonical_unit IS NULL
                    GROUP BY ALL ORDER BY feature,rows DESC)
                TO {sql_string((metrics_dir/'evidence_qc.csv').as_posix())} (HEADER)
            """)
            ...
            lost_labs = {'platelets', 'wbc', 'rbc', 'inr', 'alt', 'ast', 'alp'} - accepted_features
            if lost_labs:
                raise ValueError(f"Required eICU labs lost during cleaning: {sorted(lost_labs)}; review system/interface units")
            missing_features = [
            ...
            connection.execute(f"""
                COPY (
                    SELECT source_table, feature, itemid, raw_unit, source_system_unit, source_interface_unit, canonical_unit, unit_rule,
                    ...
                    FROM cleaned
                    GROUP BY source_table, feature, itemid, raw_unit, source_system_unit, source_interface_unit, canonical_unit, unit_rule,
        ...
        report = {
            ...
            "source_lab_unit_aliases": LAB_UNIT_ALIASES,
            ...
            "policy": {
                ...
                "specimen": "PaO2/PaCO2 require labTypeID=7 ABG category as an arterial proxy; exact specimen identity is unavailable and same-time grouping is explicitly inferred",
