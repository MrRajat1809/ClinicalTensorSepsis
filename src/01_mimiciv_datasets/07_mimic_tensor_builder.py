...
DATA_DIR = BASE_DIR / "data/processed/mimiciv"
...
def build_neq(connection, policy):
    ...
    connection.execute("""
        ...
        JOIN patients AS patient USING (stay_id)
        WHERE event.source_table = 'inputevents' AND event.record_qc_status = 'accepted'
...
def build_tensor(processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR, common_dir=COMMON_DIR,
    ...
    qc = importlib.import_module("09_mimic_master_qc")
    ...
    with tempfile.TemporaryDirectory(prefix=".tensor_", dir=output_dir) as temporary:
        ...
        paths += [source_metrics_dir / name for name in ("culture_qc.csv", "antimicrobial_qc.csv", "measurement_qc.csv")]
        paths += [Path(__file__).with_name(name) for name in
                  ("01_mimic_base_cohort.py", "02_mimic_infection_cohort.py", "03_mimic_phenotypes.py",
                   "04_mimic_temporal_extract.py", "05_mimic_temporal_clean.py", "06_mimic_sepsis3_cohort.py", "07_mimic_tensor_builder.py")]
