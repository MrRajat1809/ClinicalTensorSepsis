...
DATA = ROOT / "data/processed/mimiciv"
PROCESSED_DIR = DATA / "work"
OUT = METRICS_DIR = ROOT / "outputs/mimiciv"
...
def artifact_checks(qc, state):
    ...
    script = Path(__file__).with_name("08_mimic_saits_imputation.py")
...
def observed_replay(qc, state):
    builder = module(Path(__file__).with_name("07_mimic_tensor_builder.py"), "mimic_tensor_builder")
...
def structural_checks(connection, checks):
    ...
    check("infection_timing_contract", """
        SELECT COUNT(*) FROM candidates WHERE infection_schema_version IS DISTINCT FROM '2.1.0'
            ...
            OR diagnostic_test_names IS NULL OR LEN(diagnostic_test_names) = 0
    ...
    check("respiratory_pairing", f"""
        SELECT COUNT(*) FROM respiratory WHERE
            (pf_ratio IS NOT NULL AND (
                ...
                OR fio2_pair_source NOT IN ('same_specimen', 'preceding_chart')))
            OR (fio2_pair_source = 'same_specimen' AND (
    ...
    with tempfile.TemporaryDirectory(prefix=".cohort_audit_", dir=metrics_dir) as temporary:
        ...
        try:
            ...
            with duckdb.connect() as connection:
                ...
                connection.execute("CREATE TEMP TABLE allowed_cultures (test_name VARCHAR)")
                connection.executemany("INSERT INTO allowed_cultures VALUES (?)", [
                    (" ".join(name.upper().split()),) for name in infection_policy["diagnostic_culture_test_names"]
                ])
                add_check(checks, "diagnostic_culture_mapping", connection.execute("""
                    SELECT COUNT(*) FROM (
                        SELECT UNNEST(diagnostic_test_names) AS test_name FROM candidates
                    ) evidence LEFT JOIN allowed_cultures USING (test_name)
                    WHERE allowed_cultures.test_name IS NULL
                """).fetchone()[0])
                connection.execute("CREATE TEMP TABLE excluded_specimens (name VARCHAR)")
                connection.executemany("INSERT INTO excluded_specimens VALUES (?)", [
                    (" ".join(name.upper().split()),) for name in infection_policy["excluded_specimen_names"]
                ])
                add_check(checks, "no_surveillance_specimens", connection.execute("""
                    SELECT COUNT(*) FROM candidates JOIN excluded_specimens
                        ON REGEXP_REPLACE(UPPER(TRIM(spec_type_desc)), '\\s+', ' ', 'g') = name
                """).fetchone()[0])
    ...
    with duckdb.connect() as con:
        ...
        queries = dict(
            ...
            age_formula="""SELECT COUNT(*) FROM base WHERE age IS DISTINCT FROM
                anchor_age + EXTRACT(YEAR FROM hospital_admittime) - anchor_year""",
