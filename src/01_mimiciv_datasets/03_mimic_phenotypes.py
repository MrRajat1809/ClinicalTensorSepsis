...
def build_phenotype_cohort(mimic_dir=MIMIC_DIR, processed_dir=PROCESSED_DIR, metrics_dir=METRICS_DIR):
    ...
    diagnoses_file = mimic_dir / "hosp/diagnoses_icd.csv.gz"
    ...
    with duckdb.connect(":memory:") as connection:
        ...
        connection.execute(f"""
            ...
            SELECT infection.stay_id, diagnoses.subject_id, diagnoses.hadm_id,
                   diagnoses.seq_num, diagnoses.icd_version, diagnoses.icd_code AS source_icd_code,
                   UPPER(REPLACE(TRIM(diagnoses.icd_code), '.', '')) AS icd_code
            FROM read_csv_auto({sql_string(diagnoses_file.as_posix())}, types={{
                'subject_id': 'BIGINT', 'hadm_id': 'BIGINT', 'seq_num': 'INTEGER',
                'icd_version': 'INTEGER', 'icd_code': 'VARCHAR'
            }}) diagnoses
            INNER JOIN infection ON diagnoses.subject_id = infection.subject_id
                                AND diagnoses.hadm_id = infection.hadm_id
        """)
        connection.execute(f"""
    ...
    report = {
        ...
        "policy": {
            ...
            "alternative_diagnosis_sequence_numbers": [1, 2],
            "cci_source": "retrospective admission-wide ICD diagnoses", "cci_age_adjusted": False,
